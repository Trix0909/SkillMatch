...
class Diagram:
    def __init__(self, name, width=1200, height=900):
        ...
        self.node(
            ...
            width - 60,
            45,
            "text;html=0;fontSize=26;fontStyle=1;align=left;strokeColor=none;fillColor=none;",
        )
    ...
    def node(self, text, x, y, w=230, h=90, style=""):
        ...
        style = (
            style
            or "rounded=1;whiteSpace=wrap;html=0;fillColor=#eef3fc;strokeColor=#9ab2d9;fontColor=#233a56;fontSize=14;spacing=12;"
        )
        cell = ET.SubElement(
            self.root, "mxCell", id=key, value=text, style=style, vertex="1", parent="1"
        )
        ET.SubElement(
            cell,
            "mxGeometry",
    ...
    def edge(self, source, target, label="", style=""):
        ...
        cell = ET.SubElement(
            ...
            target=target,
            style=style
            or "edgeStyle=orthogonalEdgeStyle;rounded=0;html=0;endArrow=block;strokeColor=#7c90aa;fontColor=#425974;fontSize=12;labelBackgroundColor=#ffffff;",
        )
...
m = d.node(
    "MatchingAlgorithm\nCount terms → cap at 2 → TF-IDF\nCosine × certification × experience",
    850,
    410,
    310,
    110,
...
for source, target, label in [
    (a, b, "HTML response / form input"),
    (b, c, "Django request / response"),
    (c, db, "ORM query / save"),
    (c, m, "Eligible query and corpus"),
    (m, b, "Ranked score breakdown"),
]:
    d.edge(source, target, label)

d = Diagram("Database schema", 1400, 1000)
u = d.node(
    "auth_user\nPK id · username · email\nfirst_name · last_name\npassword hash · active · staff",
    560,
    90,
    300,
    120,
...
s = d.node(
    "Skill\nPK id · FK profile\nname · normalized_name\nFK category (optional)\nUNIQUE(profile, normalized_name)",
    70,
    650,
    330,
    155,
...
for src, tgt in [
    ...
    d.edge(src, tgt, "", "endArrow=none;html=0;strokeColor=#7c90aa;")

d = Diagram("Matching activity", 1150, 1450)
steps = [
    ("Employer submits job or search text", 400, 90),
    ("Authorize employer and job ownership", 400, 225),
    ("Load active candidate profiles\nKeep only profiles with skills, bio and portfolio", 400, 360),
    ("Concatenate skills + bio + portfolio\nPrepare employer text", 400, 515),
    ("Tokenize and remove stop words\nCap raw term frequencies at 2", 400, 670),
    ("Fit shared TF-IDF representation\nCompute cosine similarity", 400, 825),
    ("Apply profile certification multiplier\nApply profile experience multiplier", 400, 980),
    ("Sort descending, break ties by ID\nPaginate ranked candidate profiles", 400, 1135),
]
nodes = [d.node(t, x, y, 430, 95) for t, x, y in steps]
for src, tgt in zip(nodes, nodes[1:]):
    d.edge(src, tgt)
d.node(
    "Reverse workflow\nA complete seeker profile is the query.\nDocuments are open jobs from active employers.\nUse the same pipeline and profile multipliers.",
    30,
    800,
    300,
    165,
...
for src, tgt, label in [
    ...
    (user, employer, "one-to-one composition"),
    (profile, skill, "owns many"),
    (employer, job, "owns many"),
    (engine, profile, "reads"),
    (engine, job, "reads"),
]:
    d.edge(src, tgt, label)
d.node(
    "Django implements role profiles through composition\nrather than subclassing its concrete User model.",
    410,
    810,
    480,
    60,
    "text;html=0;fontSize=13;fontColor=#60748e;strokeColor=none;fillColor=none;",
)
...
def message(start, end, y, label):
    ...
    d.edge(
        ...
        q,
        label,
        "endArrow=block;html=0;fontSize=12;strokeColor=#637d9d;labelBackgroundColor=#ffffff;",
    )


for row in [
    (0, 1, 230, "1. Submit job description"),
    (1, 2, 310, "2. Check ownership and retrieve active profiles"),
    (2, 1, 390, "3. Profiles, unique skills and portfolio text"),
    (1, 3, 470, "4. Rank query against profile corpus"),
    (3, 3, 530, "5. Cap TF, vectorize, cosine, multipliers"),
    (3, 1, 620, "6. Ranked Match objects"),
    (1, 0, 735, "7. Render candidates, skills and score breakdown"),
]:
    if row[0] == row[1]:
        d.node(row[3], 910, row[2] - 15, 340, 60)
    else:
        message(*row)

d = Diagram("Interface wireframes", 1500, 1300)
frames = [
    ("Registration and login", 40, 100),
    ("Professional profile", 780, 100),
    ("Employer job posting", 40, 710),
    ("Ranked match results", 780, 710),
]
for title, x, y in frames:
    d.node(
        ...
        660,
        520,
        "rounded=0;whiteSpace=wrap;html=0;fillColor=#ffffff;strokeColor=#b6c3d4;fontColor=#29425f;fontSize=18;verticalAlign=top;spacingTop=20;",
    )
d.node(
...
d.node(
    "Candidate / Job title and summary\nMatching skill tags\nCertification and experience levels\nView profile / opportunity",
    830,
    800,
    350,
    250,
