...
const output = path.resolve('tmp/browser');
fs.mkdirSync(output, {
  recursive: true
});
...
(async () => {
  ...
  async function noOverflow() {
    assert(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth + 1), 'Horizontal page overflow: ' + page.url());
  }
  ...
  await noOverflow();
  results.push('Desktop landing renders');
  await login('demo_seeker');
  assert.equal(await page.locator('.result-card').count(), 10);
  await screenshot('recommendations-desktop');
  await noOverflow();
  await page.getByText('Why this match?', {
    exact: true
  }).first().click();
  assert(await page.getByText('Certification multiplier', {
    exact: true
  }).first().isVisible());
  ...
  await page.locator('#skill-entry').fill('python');
  await page.getByRole('button', {
    name: '+ Add skill',
    exact: true
  }).click();
  assert(await page.getByText('This skill is already added.', {
    exact: true
  }).isVisible());
  await page.locator('#skill-entry').fill('Browser test skill');
  await page.getByRole('button', {
    name: '+ Add skill',
    exact: true
  }).click();
  await page.getByRole('button', {
    name: 'Remove Browser test skill',
    exact: true
  }).click();
  ...
  assert(await page.getByText('Your profile has been saved.', {
    exact: true
  }).isVisible());
  ...
  assert(await page.getByText('Page 2 of 3', {
    exact: true
  }).isVisible());
  ...
  await noOverflow();
  await screenshot('employer-mobile');
  await page.goto(base + '/employer/candidates/?q=Python+Django');
  await noOverflow();
  await screenshot('candidates-mobile');
  await page.getByRole('button', {
    name: 'Sign out ↪',
    exact: true
  }).click();
  ...
  await noOverflow();
  await screenshot('registration-mobile');
  await page.getByLabel('Role', {
    exact: false
  }).selectOption('employer');
  assert(await page.getByLabel('Company name (employers)', {
    exact: false
  }).isVisible());
  await page.getByLabel('Role', {
    exact: false
