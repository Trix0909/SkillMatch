                    ...
                    scores.append(
                        {
                            ...
                            "sus_score": sus_score([int(row[f"q{i}"]) for i in range(1, 11)]),
                        }
                    )
            if not scores:
                raise ValueError(
                    "No participant responses found. Collect responses before scoring."
                )
        except (OSError, KeyError, ValueError) as error:
            raise CommandError(str(error))
        report = {
            "participants": len(scores),
            "mean_sus": sum(s["sus_score"] for s in scores) / len(scores),
            "scores": scores,
        }
