...
from skillmatch.evaluation import pair_fingerprint, precision_at_k, rule_relevance
from skillmatch.matching import MatchingAlgorithm
from skillmatch.views import active_jobs, profiles
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options["export_labels"]:
            ...
            self.stdout.write(
                f"Exported {len(jobs) * len(candidates)} pairs. Fill relevant (0 or 1) and reviewer after manual review. Suggestions are not ground truth."
            )
            return
        if not options["labels"]:
            raise CommandError(
                "Use --export-labels PATH first, then --labels PATH with reviewed labels."
            )
        labels = {}
            ...
            with options["labels"].open(encoding="utf-8-sig", newline="") as stream:
                for row in csv.DictReader(stream):
                    ...
                    if row["fingerprint"] != fingerprints.get(key):
                        raise ValueError(
                            "Profile or job contents changed since labelling. Export and review a fresh sheet."
                        )
        ...
        if set(labels) != expected:
            raise CommandError(
                "Labels do not match the current evaluation corpus. Export and review a fresh sheet."
            )
            ...
            rows.append(
                {
                    ...
                    "weighted": {
                        f"P@{k}": precision_at_k([m.item.pk for m in matches], relevant, k)
                        for k in (5, 10)
                    },
                    "baseline": {
                        f"P@{k}": precision_at_k([m.item.pk for m in baseline], relevant, k)
                        for k in (5, 10)
                    },
                }
            )
        report = {
            ...
            "mean": {
                method: {
                    f"P@{k}": sum(r[method][f"P@{k}"] for r in rows) / len(rows) for k in (5, 10)
                }
                for method in ("weighted", "baseline")
            },
            "jobs": rows,
        }
