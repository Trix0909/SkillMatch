...
def pair_fingerprint(profile, job):
    payload = [profile.matching_text, profile.certification_level, profile.experience_level,
               job.matching_text, job.certification_level, job.experience_level,
               [s.normalized_name for s in profile.skills.all()], job.required_skills]
...
def rule_relevance(profile, job):
    ...
    levels_met = (CERT_MULTIPLIERS[profile.certification_level] >= CERT_MULTIPLIERS[job.certification_level]
                  and EXP_MULTIPLIERS[profile.experience_level] >= EXP_MULTIPLIERS[job.experience_level])
    return coverage, coverage >= .7 and levels_met
