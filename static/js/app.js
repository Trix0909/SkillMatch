document.querySelectorAll('[data-skill-editor]').forEach(editor => {
  ...
  try {
    ...
    if (Array.isArray(data)) skills = data.filter(s => typeof s === 'string');
  } catch (_) {
    /* Server renders validation for malformed input. */
  }
  ...
  const add = () => {
    ...
    if (!value) {
      error.textContent = 'Enter a skill first.';
      input.focus();
      return false;
    }
    if (value.length > 80) {
      error.textContent = 'Use at most 80 characters per skill.';
      return false;
    }
    if (skills.some(s => normalized(s).toLowerCase() === value.toLowerCase())) {
      error.textContent = 'This skill is already added.';
      return false;
    }
    if (skills.length >= 40) {
      error.textContent = 'You can add up to 40 skills.';
      return false;
    }
    skills.push(value);
    input.value = '';
  ...
  editor.closest('form').addEventListener('submit', event => {
    ...
    if (!skills.length) {
      event.preventDefault();
      error.textContent = 'Add at least one skill.';
      input.focus();
    }
  });
  render();
...
if (role && company) {
  const update = () => {
    company.hidden = role.value !== 'employer';
    const input = company.querySelector('input');
    if (input) input.required = role.value === 'employer';
  };
